...
var palindromeChainLength = function (number) {
  ...
  let steps = 0;
  let sumPalindrome = 0;
  let reverse = parseInt(`${number}`.split("").reverse().join(""), 10);
  if (number === reverse) {
    return steps;
  }
  sum = number + reverse;
  do {
    sumPalindrome = parseInt(`${sum}`.split("").reverse().join(""), 10);
    if (sum === sumPalindrome) {
      ok = true;
    } else {
      sum = sum + sumPalindrome;
    }
    steps++;
  } while (ok != true);
  return steps;
...
//result.innerHTML = palindromeChainLength(87);
